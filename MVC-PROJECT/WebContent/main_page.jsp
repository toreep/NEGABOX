...
ArrayList<Float> reviewScore = (ArrayList<Float>)(request.getAttribute("reviewScore"));
ArrayList<Integer> heartCount = (ArrayList<Integer>)(request.getAttribute("heartCount"));
HashMap<String, Integer> MemberHeartCount = (HashMap<String, Integer>)(request.getAttribute("MemberHeartCount"));
ArrayList<MainDto> movieList = (ArrayList<MainDto>)(request.getAttribute("movieList"));
%>
...
<link href='css/MainPage(Remake).css?ss' rel='stylesheet' type='text/css'>
<link href='css/main_header_footer.css?ss' rel='stylesheet' type='text/css'>
<meta charset="UTF-8">
...
$(function(){
	...
	$()
		...
		$("button[name=movieCode]").click(function() {
			var num = $(this).val();
			...
			$.ajax({
				...
				success: function(data) {
					if(data.like) {
						...
						_this.children(".img-container").addClass("on");	/* 조건식 걸어야함 */
					} else {
						_this.children(".img-container").removeClass("on");  	/* 조건식 걸어야함 */
						_this.children('span').text(data.likeHeart);
					}
				},
				...
				<div id='main-section01' class='section main-movie on'>
					...
					<div class="bg">
						<div class="bg-pattern"></div>
							<% for(MainDto vo : movieList) {
									for(int i=0; i<=1; i++) {
									
								%>
							<img src="./movie_photo/<%=vo.getMoviePhoto() %>" alt="유체이탈자_티저 포스터.jpg" onerror="noImg(this, 'main');">
							<%
									}
									break;
							   }
	...
	<!-- 	$(".poster, .rank, .screen_type").mouseenter(function(){
		...
		$(".wrap").mouseleave(function(){
			$(this).removeClass("on");
					...
					int count = 1;
					int cot = 0;
					for(MainDto vo : movieList) {	// 4번 반복	
						%>
					<% if(count == 4) { %>
							<div class="fl mr0">
								 <a class="movie-list-info" id="movie-list-info" movieName="<%=vo.getMovieName()%>"> <!--작업중  (1)sned -->
								<img class='poster' src="./movie_photo/<%=vo.getMoviePhoto()%>"/>
								...
								<div class='wrap'>
									...
									<div class="score">
										<div class="preview">
											<p class="tit">관람평</p>
											<p class="number"><%=reviewScore.get(cot)%><span class="ir">점</span></p>
											<div style='clear:both'></div>
										</div>
									</div>
								</div>
								...
								<%
									String like_on_2 = MemberHeartCount.get(vo.getMovieCode())==1 ? "on" : "";
								%>
								
								<div class='btn-util'>
									<!-- 하늘색 : <img src='./image/ico-heart-toggle-main.png'> -->
									
									<button class='button btn-like ico-heart-toggle-gray' name="movieCode" id="movieCode" value="<%=vo.getMovieCode()%>">	
									...
									<div class='case'>
										<a href='fast_rev_before_map.jsp' class="button gblue" title='영화 예매하기'>예매</a>
									</div>
								</div>
								<div style='clear:both'></div>
							</div>
							...
							<div class="fl mr40">
								<a class="movie-list-info" id="movie-list-info" movieName="<%=vo.getMovieName()%>">
								<img class='poster' src="./movie_photo/<%=vo.getMoviePhoto()%>">
								<p class='rank'><%=count%></p>
								<div class="screen_type">
									<%if(vo.getDolby() == 1) { %>
								...
								<%
									String like_on_1 = MemberHeartCount.get(vo.getMovieCode())==1 ? "on" : "";
								%>
							...
							<div class='cell'>
								<div class="search">
									...
									<button type="button" class="btn" id="btnSearch">
										<i class="iconset ico-search-w"></i>
										검색
									</button>
								</div>
							</div>
							<div class="cell">
								<a href="movie-theater-table.jsp" title="상영시간표 보기"><i class="iconset ico-schedule-main"></i>
									상영시간표</a>
							</div>
							<div class="cell">
								<a href="javascript:void(0);" title="박스오피스 보기"><i class="iconset ico-boxoffice-main"></i> 박스오피스</a>
							</div>
							<div class="cell">
								<a href="fast_rev_before_map.jsp" title="빠른예매 보기"><i class="iconset ico-quick-reserve-main"></i>
									빠른예매</a>
							</div>
						</div>
					</div>
